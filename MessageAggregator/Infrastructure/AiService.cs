    ...
    public class AiService : IAIService
    {
        ...
        public async Task<Summary> AnalyzeAsync(IEnumerable<string> data, string chatName) // Added chatName parameter
        {
            ...
            try
            {
                ...
                // Удаляем markdown-обёртку ``` или ```json ... ```
                if (!string.IsNullOrEmpty(content))
                {
                    ...
                    if (trimmedContent.StartsWith("```"))
                    {
                        ...
                        int lastBackticks = trimmedContent.LastIndexOf("```");
                        if (firstNewline > 0 && lastBackticks > firstNewline)
                        {
                            trimmedContent = trimmedContent.Substring(firstNewline + 1, lastBackticks - firstNewline - 1).Trim();
                        }
                        else
                        {
                            // Fallback: remove all ```
                            trimmedContent = trimmedContent.Replace("```json", "").Replace("```", "").Trim();
                        }
                    }
                    ...
                    try
                    {
                        ...
                        var aiResponse = JsonConvert.DeserializeAnonymousType(trimmedContent ?? string.Empty, new { summary = "", category = "" }); // Handle potential null content
